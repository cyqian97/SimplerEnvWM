...
# Lazy import to avoid import errors if SAM3/DA3 not installed
_RewardVision = None

def _get_reward_vision():
    global _RewardVision
    if _RewardVision is None:
        from mani_skill.envs.tasks.tabletop.pick_cube_vision_reward import RewardVision
        _RewardVision = RewardVision
    return _RewardVision
...
class PickCubeEnv(BaseEnv):
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: dict):
        # Get sensor data and parameters for vision-based cube detection
        sensor_data = self._get_obs_sensor_data()
        sensor_params = self.get_sensor_params()
        
        # Format observation for RewardVision function
        vision_obs = {
            "image": {},
            "sensor_param": {}
        }
        ...
        if camera_name in sensor_data and camera_name in sensor_params:
            vision_obs["image"][camera_name] = {"rgb": sensor_data[camera_name]["rgb"]}
            vision_obs["sensor_param"][camera_name] = {
                "intrinsic_cv": sensor_params[camera_name]["intrinsic_cv"],
                "cam2world_gl": sensor_params[camera_name]["cam2world_gl"]
            }
            
            # Detect cube position using SAM3 and DA3
            RewardVision = _get_reward_vision()
            pose_cube_position = RewardVision(
                vision_obs,
                camera_name=camera_name,
                device=self.device,
                cube_world_pos=self.cube.pose.p,  # Pass cube position for calibration
            )
        ...
        tcp_to_obj_dist = torch.linalg.norm(
            pose_cube_position - self.agent.tcp_pose.p, axis=1
        )
        ...
        obj_to_goal_dist = torch.linalg.norm(
            self.goal_site.pose.p - pose_cube_position, axis=1
        )
